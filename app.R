      ...
      div(
        ...
        div(
          ...
          tags$ul(
            ...
            p("Using the number of reported cases and compliance percentage, we can estimate the total number of cases",
              "for a given week/region. We use a simple projection: reported cases \u00f7 (compliance % / 100). ",
              "For example, if there are 20 reported cases, and the compliance is 50%, then the projected total number",
              "of cases is 20 \u00f7 0.5 = 40. This projection assumes non-reporting sites have a ",
            ...
            p("Alerts are generated with a CUSUM/C2 aberration detection ",
            ...
            tags$ul(
              tags$li(strong("Handling missing weeks in baseline:"), "If one or more of the 7 baseline weeks",
              "is marked NR, or wasn't in that week's bulletin at all, the ",
              ...
              "3 have a report. Below that, the method isn't applied and the week is flagged as having ",
               strong("insufficient baseline data")),
              tags$li(strong("Flat baselines:"), "A rare or sporadic disease can have a baseline where all 7 weeks",
              "report the same value, often zero, giving a standard deviation of exactly 0. Here, a standard",
              "deviation of 1 is applied, following standard practice in the aberration-detection ",
              "literature for the same method (see the ", strong("References"), " tab).")
            )
        )
